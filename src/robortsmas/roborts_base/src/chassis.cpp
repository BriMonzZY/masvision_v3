...
Chassis::~Chassis(){
  if(heartbeat_thread_.joinable()){
    heartbeat_thread_.join();  // 等待heartbeat线程结束后(ROS停止)才能销毁实例
  }
}
...
void Chassis::ChassisInfoCallback(const std::shared_ptr<roborts_sdk::cmd_chassis_info> chassis_info)
{
  ...
  tf_broadcaster_.sendTransform(odom_tf_);  // 完成tf数据的发布
}
...
void Chassis::ChassisSpeedCtrlCallback(const geometry_msgs::Twist::ConstPtr &vel) {
  ...
  chassis_speed_pub_->Publish(chassis_speed);  // 调用SendMessage调用SendCMD，从串口发布信息
}
