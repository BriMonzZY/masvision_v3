...
bool Protocol::SendCMD(uint8_t cmd_set, uint8_t cmd_id, uint8_t receiver,
  ...
  switch (session_mode) {
    ...
    case CMDSessionMode::CMD_SESSION_0:
      ...
      memcpy(cmd_session_ptr->memory_block_ptr->memory_ptr + HEADER_LEN + CMD_SET_PREFIX_LEN, data_ptr, data_length);  // 数据

      crc_data = CRC32Calc(cmd_session_ptr->memory_block_ptr->memory_ptr, pack_length - CRC_DATA_LEN);
      memcpy(cmd_session_ptr->memory_block_ptr->memory_ptr + pack_length - CRC_DATA_LEN, &crc_data, CRC_DATA_LEN);  // 校验数据

      // send it using device
      std::cout << "data length: " << data_length << "pack length" << pack_length << std::endl;  // Debug
      for(int i = 0; i < pack_length; i++) {
          printf("%x ", cmd_session_ptr->memory_block_ptr->memory_ptr[i]);
      }
      DeviceSend(cmd_session_ptr->memory_block_ptr->memory_ptr);  // 串口发送
...
bool Protocol::DeviceSend(uint8_t *buf) {
  ...
  if (ans <= 0) {
    ...
    DLOG_ERROR << "Port send failed, send length:" << ans
               << "package length" << header_ptr->length;
  } else {
